...
run:
	...
	-e PYTHONPATH=/home/appuser/EmbodiedScan \
	-v "${PWD}":/home/appuser/EmbodiedScan \
	-w /home/appuser/EmbodiedScan \
	-v "${SCANNET_PATH}":/data \
	--shm-size 128G \
	--net host --gpus all --privileged --name embodiedscan embodiedscan:latest /bin/bash

# export SCANNET_PATH=/media/sequor/PortableSSD/scannetpp && make run

# python tools/train.py configs/occupancy/mv-occ_8xb1_embodiedscan-occ2x-11class.py --work-dir=work_dirs/scannetpp
